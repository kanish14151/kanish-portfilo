  ...
  return (
    <div className="bg-white dark:bg-slate-dark text-slate-900 dark:text-white transition-colors duration-300 overflow-hidden">
          ...
          {project.images && project.images.length > 0 ? (
            project.images.map((img, idx) => (
              <img
                ...
                src={img}
                alt={`${project.title} - ${idx + 1}`}
                className={`absolute inset-0 w-full h-full object-contain transition-all duration-700 ${idx === currentImageIndex ? (isTransitioning ? 'opacity-0' : 'opacity-100') : (idx === (currentImageIndex + 1) % project.images!.length && isTransitioning ? 'opacity-100' : 'opacity-0')
                  }`}
                style={{
                  transition: 'transform 0.7s ease-in-out, opacity 0.7s ease-in-out',
      ...
      <div className="p-6 md:p-8">
        ...
        <div className="mb-8">
          <p className="text-gray-600 dark:text-secondary leading-relaxed text-base md:text-lg whitespace-pre-line">
        ...
        <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-white/10">
          <a
            ...
            className="flex-1 bg-slate-900 dark:bg-white text-white dark:text-black px-4 py-3.5 rounded-xl hover:bg-book-cloth hover:text-white transition-all duration-300 font-bold text-sm shadow-lg hover:shadow-xl hover:-translate-y-0.5 text-center"
          >
            Live Demo
          </a>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 bg-transparent border border-gray-200 dark:border-white/20 text-slate-900 dark:text-white px-4 py-3.5 rounded-xl hover:border-slate-900 dark:hover:border-white hover:bg-gray-50 dark:hover:bg-white/5 transition-all duration-300 font-bold text-sm hover:-translate-y-0.5 text-center"
          >
            View Code
          </a>
        </div>
      ...
      <div className="container mx-auto">
        <FadeIn>
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <div>
              <h1 className="text-5xl font-serif font-bold text-slate-900 dark:text-primary mb-4">Projects</h1>
            ...
            <div className="flex flex-wrap gap-2 mt-6 md:mt-0">
              {categories.map(cat => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-300 border ${filter === cat
